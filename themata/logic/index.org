...
imagine a more admirable person than the [[https://en.wikipedia.org/wiki/Gerhard_Gentzen#Life_and_career][Nazi Gentzen]] having done that work).
...
on assumption =[1]=, we rewrite $B$ as $A \& B \supset B$. We do the same
...
is valid just because $(A \supset C) \supset ((B \& A) \supset C)$. The "meaning
explanation" for each structural rule is given the same way. On the one hand,
the "structural rules" appear to be obfuscations of the logical structures
articulated in our object logic. On the other hand, *repetition of "the same"
structures in various media is near to the essence of what it is to be logical*,
