...
a description so vague that no one can tell if it's wrong. We not seek to
...
Cf. the theory of [[https://en.wikipedia.org/wiki/Institution_(computer_science)][institutions]] and the project of [[https://en.wikipedia.org/wiki/Universal_logic][universal logic]].
* Links
